...
class AA(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        ...
        A = transform_func(X, archetypes, max_iter=self.max_iter, tol=self.tol, **method_params)
        return A
...
def nnls_transform(X, archetypes, *, max_iter, tol, **params):
    return nnls(X, archetypes, **params)
...
def nnls_fit_transform(X, A, B, archetypes, *, max_iter, tol, verbose, **params):
    ...
    for i in range(1, max_iter + 1):
        A = nnls(X, archetypes, **params)
        B = nnls(np.linalg.pinv(A) @ X, X, **params)
